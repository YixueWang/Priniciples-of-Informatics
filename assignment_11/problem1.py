...
def kdtreeApproach(intersections, tripLocations):
    ...
    idx_start =intersections.query(tripLocations)
    for item in idx_start[1]:
        if item in counts:
            counts[item] +=1
        else:
            counts[item] = 1
...
def plotResults(intersections, counts):
    ...
    for item in counts:
        ...
        r_list.append(counts[item])

    output_file("plot for problem1.html", title="Plots for problem1 ")

    scatter(lng_list, lat_list, color='red',alpha = 0.5, size = r_list, fill_alpha=0.6,line_color=None, title = "Plots for problem1")
